...
test('properties', async ({ page }) => {
  ...
  await browser.table.expectToHaveRowValues([
    'NTLM.challengeWhether to authenticate via NTLM by responding to auth-challenges (deprecated). Used by the ch.ivyteam.ivy.webservice.exec.cxf.feature.NTLMAuthenticationFeature'
  ]);

  await browser.table.row(0).locator.click();
  await expect(browser.info.content).toHaveText(
    'InfoWhether to authenticate via NTLM by responding to auth-challenges (deprecated). Used by the ch.ivyteam.ivy.webservice.exec.cxf.feature.NTLMAuthenticationFeatureDefault value: true'
  );
});
